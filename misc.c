...
struct EMULINK {
	...
	// fake floppy drive
#ifdef RP2350_BUILD
	FIL fdd[2];       // FatFS file handles
	int fdd_open[2];  // Track if file is open
#else
	FILE *fdd[2];
#endif
	int fdfmti[2];
...
int emulink_attach_floppy(EMULINK *e, int i, const char *filename)
{
	if (i >= 0 && i < 2) {
		...
		if (filename) {
			...
			FRESULT res = f_open(&e->fdd[i], path, FA_READ | FA_WRITE);
			if (res != FR_OK) {
				return -1;
			}
			e->fdd_open[i] = 1;

			// Get file size and match format
			int size = f_size(&e->fdd[i]);
	...
	case 0x102:
		if (e->argi == 3) {
			...
			if (e->args[0] < 2) {
				...
				FRESULT res = f_lseek(&e->fdd[e->args[0]], 512 * lba);
				if (res != FR_OK) {
					e->status = -EIO;
					e->cmd = -1;
				} else {
	...
	case 0x102: // floppy write
		...
		if (e->argi == 3) {
			...
			if (len > e->dataleft)
				break;
			UINT bw;
			FRESULT res = f_write(&e->fdd[e->args[0]], buf, len, &bw);
	...
	case 0x101: // floppy read
		...
		if (e->argi == 3) {
			...
			if (len > e->dataleft)
				break;
			UINT br;
			FRESULT res = f_read(&e->fdd[e->args[0]], buf, len, &br);
			if (res != FR_OK || (int)br != len)
				break;
			e->dataleft -= len;
			if (e->dataleft == 0) {
				e->cmd = -1;
